...
class AdaptiveTuningRequest:
    ...
    def __post_init__(self):
        ...
        total=_bound(self.max_total_drift_cents,'max_total_drift_cents',0.,600.);step=_bound(self.max_step_cents,'max_step_cents',0.,120.)
        search=_bound(self.search_step_cents,'search_step_cents',.1,60.)
        ...
        object.__setattr__(self,'desired_tension',_bound(self.desired_tension,'desired_tension',0.,1.))
        object.__setattr__(self,'max_total_drift_cents',total);object.__setattr__(self,'max_step_cents',step);object.__setattr__(self,'search_step_cents',search)
        object.__setattr__(self,'min_separation_cents',sep);object.__setattr__(self,'min_source_confidence',_bound(self.min_source_confidence,'min_source_confidence',0.,1.))
    def to_dict(self):return {'kind':'AdaptiveTuningRequest','version':ADAPTIVE_VERSION,'method_id':ADAPTIVE_METHOD_ID,'voices':[v.to_dict() for v in self.voices],
        ...
        'root_lock':self.root_lock,'max_total_drift_cents':self.max_total_drift_cents,'max_step_cents':self.max_step_cents,
        'search_step_cents':self.search_step_cents,'min_separation_cents':self.min_separation_cents,'min_source_confidence':self.min_source_confidence,
        'max_passes':self.max_passes,'weights':self.weights.to_dict(),'amplitude_policy':'immutable; adaptive tuning changes pitch only'}
