...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            headers=page.locator('thead').inner_text();assert 'Requested' in headers and 'Estimated' in headers and 'Realised' in headers
            ...
            page.goto(base+'/');expect(page.locator('body')).to_be_visible();assert page.locator('a[href="/inspector"]').count()==1
            assert not errors,errors;browser.close()
        report={'method':'zg023-browser-v1','sample_rate_hz':args.sample_rate,'checks':'passed','page_errors':errors,'policy':{'analysis_auto_apply':False,'stale_result_rejected':True,'compose_root_remains_available':True}}
