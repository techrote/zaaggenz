...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            tid=next(iter(server.listening.trials));truth=server.listening.trials[tid].to_dict()['abx_truth'];page.locator('#choice').select_option(truth)
            ...
            result=server.listening.results[tid][-1].to_dict();assert result['abx_correct'] is True;assert result['x_replay_count']==1;assert result['confidence']==73;assert result['effort']==27;assert set(result['ratings'])=={'liking','sound_quality'};assert 'abx_correct' not in result['ratings'];assert len(result['annotations'])==1
            with page.expect_download() as dl:page.locator('#export').click()
            bundle_path=a.out/'trial.json';dl.value.save_as(bundle_path);bundle=json.loads(bundle_path.read_text());assert bundle['manifest']['id']==tid;assert bundle['results'][-1]['abx_correct'] is True;assert len(bundle['stimuli'])==2
        ...
        report={'checks':'passed','design':'abx','trial_id':tid,'abx_correct':True,'ratings_fields':sorted(result['ratings']),'confidence':result['confidence'],'effort':result['effort'],'x_replay_count':result['x_replay_count'],'annotation_count':len(result['annotations']),'compose_independent':True,'page_errors':errors}
        (a.out/'browser-report.json').write_text(json.dumps(report,indent=2,sort_keys=True)+'\n');print(json.dumps({'checks':'passed','trial_id':tid}))
    finally:
