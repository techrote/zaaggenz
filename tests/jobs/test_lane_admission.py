...
class LaneAdmissionTests(unittest.TestCase):
    ...
    def tearDown(self):
        for scheduler in self.schedulers:scheduler.shutdown(cancel=True,timeout=2)
    ...
    def test_exact_background_capacity_is_feasible(self):
        ...
        self.assertEqual(scheduler.wait(job,1).state,'completed')
    ...
    def test_temporarily_blocked_feasible_background_job_remains_queueable(self):
        ...
        first=scheduler.submit(JobClass.ANALYSIS,R,lambda ctx:(started.set(),release.wait(2),1)[2],estimated_memory_bytes=40)
        ...
        release.set()
        self.assertEqual(scheduler.wait(first,1).state,'completed')
        self.assertEqual(scheduler.wait(second,1).state,'completed')
