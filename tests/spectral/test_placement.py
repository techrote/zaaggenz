...
class PlacementTests(unittest.TestCase):
    def test_family_order_is_explicit_and_only_spectral_position_changes(self):
        a,b=stages()
        rows={p:PlacementRequest(spectral(),p,a,b).to_dict() for p in ('pre','inter','post')}
        self.assertEqual(rows['pre']['order'],['spectral','stage_a','stage_b'])
        self.assertEqual(rows['inter']['order'],['stage_a','spectral','stage_b'])
        self.assertEqual(rows['post']['order'],['stage_a','stage_b','spectral'])
        for p in rows:
    ...
    def test_placement_family_is_deterministic_and_order_changes_output(self):
        ...
        self.assertGreater(np.max(np.abs(first['pre'].audio-first['post'].audio)),1e-5)
        self.assertGreater(np.max(np.abs(first['inter'].audio-first['post'].audio)),1e-5)
        for result in first.values():
            self.assertEqual(result.audio.shape,x.shape);self.assertTrue(np.isfinite(result.audio).all())
            self.assertEqual(result.diagnostics['normalization'],'none');self.assertEqual(result.diagnostics['master_gain_db'],0.)

    def test_full_identity_aligns_all_variants_in_latency_and_gain(self):
        x=tone(445.);a,b=stages(mix=0.,factor=4)
        family=run_family(x,SR,spectral(amount=0.),a,b)
        for p,result in family.items():
            np.testing.assert_array_equal(result.audio,x)
            self.assertTrue(result.diagnostics['identity_path']);self.assertEqual(result.diagnostics['declared_latency_samples'],0)
            self.assertEqual(len(result.audio),len(x))

    def test_legacy_alias_mode_remains_a_separate_one_x_option(self):
        a,b=stages(factor=1,mode='legacy');result=run_placement(tone(445.),SR,PlacementRequest(spectral(),'inter',a,b))
        spec=result.request.to_dict();self.assertEqual(spec['stage_a']['type_id'],'core.tanh.v1')
        self.assertEqual(spec['stage_b']['type_id'],'core.hard_clip.v1');self.assertEqual(spec['stage_a']['filter']['id'],'legacy-direct-1x')
    ...
    def test_inspection_persists_graph_nodes_and_spectral_decisions(self):
        ...
        self.assertEqual(payload['request']['placement'],'pre');self.assertEqual(len(payload['graph_nodes']),2)
        self.assertEqual(payload['graph_nodes'][0]['type_id'],'core.tanh_aa.v1');self.assertTrue(payload['spectral']['frames'])
        self.assertIn('post-spectral',result.taps);self.assertIn('output',result.taps)
