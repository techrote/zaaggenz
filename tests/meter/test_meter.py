...
def beats(rows): return [fraction(row['beat']) for row in rows]
...
class ValidationTests(unittest.TestCase):
    ...
    def test_roundtrip_snapshot_and_hash(self):
        ...
        self.assertEqual(MeterPlan.from_json(json.dumps(self.plan.to_dict())).sha256, original)
    ...
    def test_unknown_version_fields_nonfinite_and_bool_rejected(self):
        ...
        self.bad(lambda d: d['bindings'][0].update(values=[True]))
    ...
    def test_nested_relation_period_window_reset_and_phase_contracts(self):
        self.bad(lambda d: d['clocks'][1]['relation'].update(period_numerator=3, period_denominator=2), 'integer period multiple')
    ...
    def test_cross_clock_is_explicitly_nonbinary_and_reduced(self):
        self.bad(lambda d: (d['clocks'][3].update(period_beats='1/2'), d['clocks'][3]['relation'].update(pulses=2, reference_cycles=1)), 'power-of-two')
...
class ClockTests(unittest.TestCase):
    ...
    def test_124_nested_ticks_are_phase_consistent_and_reentry_is_exact(self):
        ...
        articulation, bounce, sway = [set(beats(all_ticks[name])) for name in ('articulation','bounce','sway')]
        ...
        for name in ('articulation','bounce','sway'):
            ...
            first = next(r for r in rows if fraction(r['beat']) >= 32)
            self.assertEqual((first['beat'], first['phase_origin_beat']), ('32/1','32/1'))
        self.assertEqual(len(articulation), len(bounce)*2); self.assertEqual(len(bounce), len(sway)*2)
    ...
    def test_shared_nested_beats_map_to_identical_samples_at_20_and_360_bpm(self):
        for bpm in ('20/1','360/1'):
            ...
            mapped = {name:{fraction(r['beat']):r['sample'] for r in generate_ticks(self.plan,name,tm)} for name in ('articulation','bounce','sway')}
    ...
    def test_tempo_changes_do_not_change_beat_phase_hierarchy(self):
        ...
        sets = {name:set(beats(generate_ticks(self.plan,name,tm))) for name in ('articulation','bounce','sway')}
        self.assertTrue(sets['sway'] <= sets['bounce'] <= sets['articulation'])
        rows = generate_ticks(self.plan,'sway',tm)
        intervals = [(fraction(a['beat']), b['sample']-a['sample']) for a,b in zip(rows,rows[1:]) if a['phase_segment_index']==b['phase_segment_index']]
        self.assertNotEqual({d for b,d in intervals if b<8}, {d for b,d in intervals if Fraction(8)<=b<Fraction(16)})

    def test_noninteger_sample_boundaries_use_frozen_round_once_policy(self):
        tm = time_map(sr=44100, origin=0, tempos=[('0/1','137/1')]); rows = generate_ticks(self.plan,'articulation',tm)
        self.assertGreater(sum(fraction(r['exact_sample']).denominator != 1 for r in rows), 10)
        for r in rows:
    ...
    def test_cross_3_2_is_not_half_or_double_time(self):
        relation = clock_relation(self.plan,'cross-3-2'); self.assertEqual(relation['cross_label'],'3:2'); self.assertFalse(relation['binary_nested'])
        cross = beats(generate_ticks(self.plan,'cross-3-2')); sway = set(beats(generate_ticks(self.plan,'sway')))
        self.assertTrue(any(b.denominator==3 for b in cross)); self.assertTrue(any(b not in sway for b in cross)); self.assertIn(Fraction(2), set(cross)&sway)

    def test_long_cycle_phase_reset_restarts_from_declared_phase(self):
        rows = generate_ticks(self.plan,'long-cycle'); first = next(r for r in rows if fraction(r['beat']) >= 40)
        self.assertEqual((fraction(first['beat']), first['phase_origin_beat']), (Fraction(321,8),'40/1'))

    def test_control_schedule_keeps_anchor_and_variable_lanes_separate(self):
        rows = control_schedule(self.plan); anchor=[r for r in rows if r['stability']=='anchor']; variable=[r for r in rows if r['stability']=='variable']
        self.assertTrue(anchor and variable); self.assertTrue(all(r['clock_id']=='sway' and r['stable_clock'] for r in anchor))
...
class OverlayTests(unittest.TestCase):
    def setUp(self): self.plan = nested_124_cross32()
    def candidate(self, report, clock): return next(r for r in report['candidates'] if r['clock_id']==clock)

    def test_fast_measurements_can_support_slower_nested_levels_simultaneously(self):
        report = periodicity_overlay(self.plan, measured_beats=[r['beat'] for r in generate_ticks(self.plan,'articulation')], tolerance_fraction='1/16', support_threshold=.95)
        for clock in ('articulation','bounce','sway'):
            self.assertEqual(self.candidate(report,clock)['tick_coverage'],1.0); self.assertIn(clock,report['ambiguity_clock_ids'])
        ...
        self.assertNotIn('cross-3-2',report['ambiguity_clock_ids']); self.assertTrue(report['extra_events_do_not_penalize_tick_coverage'])
    ...
    def test_cross_measurements_identify_cross_clock_without_calling_it_binary(self):
        report = periodicity_overlay(self.plan, measured_beats=[r['beat'] for r in generate_ticks(self.plan,'cross-3-2')], tolerance_fraction='1/32', support_threshold=.95)
        self.assertEqual(self.candidate(report,'cross-3-2')['tick_coverage'],1.0); self.assertIn('cross-3-2',report['ambiguity_clock_ids'])

    def test_sample_overlay_uses_time_map_and_reports_rounding_range(self):
        tm=time_map(sr=44100,origin=29,tempos=[('0/1','137/1'),('16/1','181/1')]); measured=[r['sample'] for r in generate_ticks(self.plan,'sway',tm)]
        report=periodicity_overlay(self.plan,time_map=tm,measured_samples=measured,tolerance_fraction='1/32',support_threshold=.95); sway=self.candidate(report,'sway')
        self.assertEqual(sway['tick_coverage'],1.0); self.assertLess(sway['sample_interval_min'],sway['sample_interval_max'])

    def test_zg012_feature_timeline_adapter_preserves_multiple_hypotheses(self):
        tm=time_map(sr=48000,origin=0,tempos=[('0/1','200/1')]); samples=[r['sample'] for r in generate_ticks(self.plan,'sway',tm)]
        frames=[SimpleNamespace(anchor_sample=s,valid=True,spectral_flux=1.0) for s in samples]; feature=SimpleNamespace(sample_rate_hz=48000,frames=frames)
        self.assertEqual(feature_flux_samples(feature,.5),samples)
        self.assertEqual(self.candidate(overlay_feature_flux(self.plan,tm,feature,.5,tolerance_fraction='1/32',support_threshold=.95),'sway')['tick_coverage'],1.0)
        with self.assertRaisesRegex(MeterError,'sample rates differ'): overlay_feature_flux(self.plan,tm,SimpleNamespace(sample_rate_hz=44100,frames=frames),.5)

    def test_measurement_input_errors_are_explicit(self):
        with self.assertRaisesRegex(MeterError,'exactly one'): periodicity_overlay(self.plan,measured_beats=[],measured_samples=[])
        with self.assertRaisesRegex(MeterError,'strictly increasing'): periodicity_overlay(self.plan,measured_beats=['1/1','1/1'])
        with self.assertRaisesRegex(MeterError,'tolerance'): periodicity_overlay(self.plan,measured_beats=[],tolerance_fraction='3/4')
