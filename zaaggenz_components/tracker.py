...
TRANSIENT_DETECTOR_POLICY='short-flux-plus-derivative-fail-closed-v1'
TRANSIENT_PRIMARY='zg-multiresolution-features-v1'
...
def _transient_provenance(mode,primary_status,fallback_class='none',fallback_reason='none',transform_eligibility='normal'):
    return dict(policy=TRANSIENT_DETECTOR_POLICY,primary=TRANSIENT_PRIMARY,mode=mode,primary_status=primary_status,
                fallback_class=fallback_class,fallback_reason=fallback_reason,transform_eligibility=transform_eligibility)

def _transient_mask(a,sr,spec):
    n=len(a);mask=np.zeros(n,dtype=np.float32)
    if n==0:
        return mask,0,_transient_provenance('not-run-empty-input','not-applicable',transform_eligibility='preserve-only')
    mono=np.mean(a,axis=1)
    # Deliberately no exception fallback: implementation/dependency/resource faults from
    # multiresolution analysis retain their native class and traceback instead of silently
    # changing transient ownership. The only fallback below is a normal analysis abstention.
    ...
    anchors=[]
    if len(vals):
        provenance=_transient_provenance('short-flux-plus-derivative','ok')
        ...
        # A successful primary analysis can explicitly abstain when it has no fully
        # supported finite flux evidence (for example silence or very short material).
        # The derivative sentinel still reports obvious sample-local attacks, but lack of
        # primary evidence may never grant transform permission.
        provenance=_transient_provenance('derivative-only-primary-abstained','abstained','analysis-abstention',
                                         'no-full-support-spectral-flux-frames','preserve-only')
    if n>2:
    ...
    return mask,len(set(anchors)),provenance

def _track_bundle(source,sr,tracks,spec,r,transient_mask,residual,transient,transient_provenance):
    ...
    transform_allowed=transient_provenance['transform_eligibility']=='normal'
    for idx,t in enumerate(tracks,1):
        ...
        frames=[]
        for row in t['rows']:
            action='transform' if transform_allowed and continuity=='continuous' and row['confidence']>=spec.transform_confidence and transient_mask[row['anchor']]<.5 else 'preserve'
    ...
    conf=dict(spec.metadata());conf.update(window_samples=r.spec.window_samples,hop_samples=r.spec.hop_samples,fft_samples=r.spec.fft_samples,
        transient_detector_policy=transient_provenance['policy'],transient_detector_mode=transient_provenance['mode'],
        transient_detector_primary_status=transient_provenance['primary_status'],transient_detector_fallback_class=transient_provenance['fallback_class'],
        transient_detector_fallback_reason=transient_provenance['fallback_reason'],transient_detector_transform_eligibility=transient_provenance['transform_eligibility'])
...
def analyse_components(x,sample_rate_hz,spec=ComponentTrackerSpec()):
    ...
    if n==0:
        dummy=type('R',(),{'spec':type('S',(),{'window_samples':_pow2(sample_rate_hz*spec.window_seconds),'hop_samples':1,'fft_samples':_pow2(sample_rate_hz*spec.window_seconds)*spec.fft_factor})()})();z=np.zeros_like(source32);mask=np.zeros(0,dtype=np.float32);transient_provenance=_transient_provenance('not-run-empty-input','not-applicable',transform_eligibility='preserve-only');bundle,_,_=_track_bundle(source32,sample_rate_hz,[],spec,dummy,mask,z,z,transient_provenance);src=source32[:,0] if mono else source32
        return ComponentAnalysis(bundle,src,src.copy(),src.copy(),src.copy(),mask,dict(method=METHOD,tracks=0,tracked_frames=0,transform_frames=0,abstained_frames=0,ambiguous_tracks=0,transient_fraction=0.,reconstruction_rms_error=0.,transient_detector=transient_provenance),sample_rate_hz)
    r,frames,abstained=_candidate_frames(a,sample_rate_hz,spec);tracks=_track(frames,spec);mask,onsets,transient_provenance=_transient_mask(a,sample_rate_hz,spec);zero=np.zeros_like(source32);temp,_,_=_track_bundle(source32,sample_rate_hz,tracks,spec,r,mask,zero,zero,transient_provenance)
    source_view=source32[:,0] if mono else source32;raw=reconstruct_components(temp,source=source_view,sample_rate_hz=sample_rate_hz);raw2=raw[:,None] if raw.ndim==1 else raw;sinusoidal=np.asarray(raw2*(1-mask[:,None]),dtype=np.float32);transient=np.asarray(source32*mask[:,None],dtype=np.float32);residual=np.asarray(source32-sinusoidal-transient,dtype=np.float32);bundle,transform_frames,ambiguous_tracks=_track_bundle(source32,sample_rate_hz,tracks,spec,r,mask,residual,transient,transient_provenance)
