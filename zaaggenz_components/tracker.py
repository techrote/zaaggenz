...
def _pow2(value):
    return int(2**round(math.log2(max(32,value))))
...
def _asset(a,sr,level='source'):
    ...
    return dict(kind='AudioAssetRef',version='1.0.0',content_sha256=hashlib.sha256(payload).hexdigest(),
                identity_domain='pcm-f32le-interleaved-v1',sample_rate_hz=sr,channels=ch,
                channel_layout='mono' if ch==1 else 'stereo-lr',frame_count=frames,level_domain=level,sample_policy='unclamped_float')
...
def _parabolic_frequency(mag,k,sr,nfft):
    ...
    delta=float(np.clip(delta,-.5,.5));return (k+delta)*sr/nfft
...
def _fit_candidates(a,sr,result,i,frequencies,base_confidences,ambiguity_flags):
    ...
    if start>=end:return []
    win_n=s1-s0;w=signal.windows.hann(win_n,sym=False);local=np.arange(start,end)-s0
    weight=np.sqrt(np.maximum(w[local],0));tau=(np.arange(start,end,dtype=np.float64)-anchor)/sr
    cols=[]
    ...
    penalty=1.0 if cond<1e4 else max(.05,1e4/max(cond,1e4))
    rows=[]
    for j,f in enumerate(frequencies):
        c=coef[2*j];s=coef[2*j+1];amps=np.hypot(c,s);ph=np.arctan2(-s,c)
        conf=float(np.clip(base_confidences[j]*penalty,0,1))
        rows.append(dict(frame=i,anchor=anchor,start=s0,end=s1,frequency_hz=float(f),
                         amplitudes=tuple(float(max(0,x)) for x in amps),
                         phases=tuple(float((x+math.pi)%(2*math.pi)-math.pi) for x in ph),
                         confidence=conf,ambiguous=bool(ambiguity_flags[j]),condition=cond))
    return rows
...
def _candidate_frames(a,sr,spec):
    ...
    for i in range(len(r.anchors)):
        ...
        for k in ks:
            ...
            snr=20*math.log10(max(mag[k],1e-30)/floor)
            conf=np.clip((snr-spec.min_snr_db)/30.,0,1)*np.clip(1-flat/max(spec.max_flatness,1e-9),0,1)
            if conf>=spec.min_confidence:fs.append(float(f));confs.append(float(conf))
        order=np.argsort(fs) if fs else []
        fs=[fs[j] for j in order];confs=[confs[j] for j in order]
        amb=[False]*len(fs)
        for j in range(len(fs)-1):
            cents=1200*abs(math.log2(fs[j+1]/fs[j]))
            if cents<spec.ambiguity_cents:amb[j]=amb[j+1]=True
        fitted=_fit_candidates(a,sr,r,i,fs,confs,amb)
        rows.append(fitted)
...
def _prediction(track,frame):
    ...
    if len(track['rows'])>=2:
        prev=track['rows'][-2];steps=max(1,last['frame']-prev['frame']);ahead=max(1,frame-last['frame'])
        ratio=max(.5,min(2.,last['frequency_hz']/prev['frequency_hz']))
        pred*=ratio**(ahead/steps)
    return pred
...
def _track(frame_rows,spec):
    ...
    for frame,cands in enumerate(frame_rows):
        ...
        # One candidate near multiple predictions is a crossing/merge ambiguity.
        for j,c in enumerate(cands):
            close=[i for i,p in enumerate(preds) if 1200*abs(math.log2(c['frequency_hz']/p))<spec.ambiguity_cents]
            if len(close)>1:
                c['ambiguous']=True
        ...
        if active and cands:
            ...
            for i,j in zip(rr,cc):
                ...
                row=deepcopy(cands[j]);active[i]['rows'].append(row);active[i]['missed']=0
                active[i]['ambiguous']|=row['ambiguous'];active[i]['had_gap']|=(frame-active[i]['last_frame']>1);active[i]['last_frame']=frame
                matched_t.add(i);matched_c.add(j)
        survivors=[]
    ...
    return [t for t in done if len(t['rows'])>=spec.min_track_frames]
...
def _track_bundle(source,sr,tracks,spec,r,transient_mask,residual,transient):
    source1=source[:,0] if source.shape[1]==1 else source
    residual1=residual[:,0] if residual.shape[1]==1 else residual
    transient1=transient[:,0] if transient.shape[1]==1 else transient
    asset=_asset(source1,sr);resasset=_asset(residual1,sr);transasset=_asset(transient1,sr)
    exported=[];transform_frames=0;ambiguous_tracks=0
    for idx,t in enumerate(tracks,1):
        ...
        for row in t['rows']:
            ...
            frames.append(dict(support=dict(start_sample=row['start'],end_sample=row['end'],anchor_sample=row['anchor'],padding='zero'),
                               frequency_hz=row['frequency_hz'],amplitudes=list(row['amplitudes']),phases_radians=list(row['phases']),
                               confidence=row['confidence'],action=action))
    ...
    bundle=dict(kind='PartialTrackBundle',version='1.0.0',asset=asset,
                method=dict(id=METHOD,version='1.0.0',configuration=conf),
                phase_convention='cosine-at-anchor-radians-v1',channel_policy='shared-frequency-independent-channel-coefficients',
                data_origin='estimated',tracks=exported,residual_asset=resasset,transient_asset=transasset,
                remainder_policy='additive-owned-remainders-v1')
    return Contract(bundle),transform_frames,ambiguous_tracks
...
def analyse_components(x,sample_rate_hz,spec=ComponentTrackerSpec()):
    ...
    sinusoidal=np.asarray(raw2*(1-mask[:,None]),dtype=np.float32)
    transient=np.asarray(source32*mask[:,None],dtype=np.float32)
    residual=np.asarray(source32-sinusoidal-transient,dtype=np.float32)
    bundle,transform_frames,ambiguous_tracks=_track_bundle(source32,sample_rate_hz,tracks,spec,r,mask,residual,transient)
    reconstruction=np.asarray(sinusoidal,dtype=np.float64)+np.asarray(transient,dtype=np.float64)+np.asarray(residual,dtype=np.float64)
    err=float(np.sqrt(np.mean((reconstruction-np.asarray(source32,dtype=np.float64))**2))) if source32.size else 0.
    tracked=sum(len(t['rows']) for t in tracks);source_rms=float(np.sqrt(np.mean(source32.astype(np.float64)**2))) if source32.size else 0.
    diag=dict(method=METHOD,tracks=len(tracks),tracked_frames=tracked,transform_frames=transform_frames,
              abstained_frames=abstained,ambiguous_tracks=ambiguous_tracks,detected_transient_onsets=onsets,
              transient_fraction=float(np.mean(mask)) if n else 0.,source_rms=source_rms,
              sinusoidal_rms=float(np.sqrt(np.mean(sinusoidal.astype(np.float64)**2))) if sinusoidal.size else 0.,
              residual_rms=float(np.sqrt(np.mean(residual.astype(np.float64)**2))) if residual.size else 0.,
              reconstruction_rms_error=err,window_samples=r.spec.window_samples,hop_samples=r.spec.hop_samples,fft_samples=r.spec.fft_samples)
