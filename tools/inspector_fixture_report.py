"""Generate deterministic ZG-023 inspector evidence without applying the transform."""
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--sample-rate',type=int,default=48000);p.add_argument('--out',type=Path,required=True);args=p.parse_args();service=InspectorService(args.sample_rate)
    try:
        ...
        report={'scope':'deterministic engineering/UI evidence; no listening or preference claim','platform':platform.platform(),'python':sys.version,'sample_rate_hz':args.sample_rate,
                'state_policy':state['policy'],'slot_identities':state['slots'],'snapshot_id':snap['snapshot_id'],'stage_order':snap['stage_order'],
