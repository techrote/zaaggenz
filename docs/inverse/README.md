...
The core inverse contracts remain `1.0.0`, finite, serializable and canonically hashed through the accepted tagged exact-binary-rational `zg-c14n-v1` implementation. Persisted `InverseCandidate` evidence is the deliberate exception: issue #134 migrates candidates to **2.0.0** with a mandatory `InverseSearchBinding` **1.0.0** because Candidate 1.0.0 did not retain enough authenticated material to verify its provenance after reopen. Unknown/missing fields and unsupported versions fail rather than migrate silently. See [candidate provenance](CANDIDATE_PROVENANCE.md) for the binding, verification and legacy replay rule.
...
| `RenderTrace`, `Candidate` | Source/pre-master/post-gain/output provenance, reproducibility state, fit measurements, validation state, editable recipe, vector and lineage. Candidate 2.0.0 also binds the canonical request/method/environment/fitting-input provenance envelope. |
...
Candidate parsing checks parameter state against its editable recipe, candidate/render/feature identities, aggregate against individual window measurements, scalar/Pareto scaling against the retained components, and Candidate 2.0.0's complete persisted search binding. Source revision, base recipe, target asset, implementation, environment, feature/render method, stage and parent projections must agree with that binding. These are content-addressed consistency guarantees rather than secret-key authentication; exact replay remains the authority on actual rendering. Legacy Candidate 1.0.0 records fail closed and must be replayed/regenerated because their missing binding cannot be safely reconstructed from duplicated SHA fields.
...
Search identity binds target/source/recipe identities, state domain/bounds, stage/method, seed, fit windows, budget, objective/validation policies, exact relevant source-file hashes and feature configuration. Its historical `zaaggenz.inverse-search-v1` domain is retained. Candidate 2.0.0 additionally binds its parameter state, editable recipe and the SHA-256 of the complete persisted search binding, which includes the numerical environment manifest. The environment records Python, NumPy, SciPy, platform, byte order and BLAS backend; numeric work is limited to one thread. Source text hashes normalize checkout line endings. Restart the process after implementation changes; hot code replacement is not supported.

Same-environment cold reruns and verified resume are exact. **Cross-platform equivalence is a tolerance-based engineering comparison, not interchangeable identities or resumability.** CI records each platform's exact identities and compares portable recipe definitions/metrics with predeclared absolute `1e-7` and relative `1e-5` tolerances. It does not overwrite hashes to force equality. Environment-specific candidate/search-binding hashes are expected to differ when the recorded environment differs; portable recipe/measurement semantics remain the cross-platform comparison surface.
