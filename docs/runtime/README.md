...
Compatibility is endpoint-owned rather than ambient. A workspace normally authorizes only its own exact current release identity. The sole intentional cross-workspace mutation path is Listening -> Timeline render: the Listening frontend must render the exact Compose source before freezing a stimulus, and its release digest includes the Timeline protocol owner. Timeline therefore accepts the current Timeline or current Listening release for Timeline mutations. Inspector and Vocal release IDs are not accepted as Timeline mutation identities, and no release identity substitutes for the existing session/trusted tokens. This bounded caller rule prevents the cache/release gate from accidentally becoming a cross-workspace capability.
...
`tests/runtime/test_web_release.py` adds adversarial and boundary coverage for content-derived identity, all four bootstrap/static policies, exact and obsolete asset fingerprints, stale CSS, missing/wrong/exact browser mutation headers, fail-before-mutation behavior, an already-loaded old release after a backend identity change, non-browser API compatibility, and every retained standalone wrapper. `tests/runtime/test_web_release_cross_workspace.py` separately proves that the current Listening release can use the intentional Timeline-render dependency while another current sidecar release is rejected before Timeline state mutation.
